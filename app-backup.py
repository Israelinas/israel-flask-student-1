    ...
    if request.method == "POST":
        try:
            ...
            SELECT students.student_id
                 ...
                 , courses.name FROM active_courses
            ...
            return render_template("attendance.html", students=students, courses=courses,
                                   teachers=teachers, message=message,     current_date=current_date)
        except:
            error_message = "Please select a course"
            return render_template("attendance.html", courses=courses, teachers=teachers, message=message, error_message=error_message)
  
    return render_template("attendance.html", courses=courses, message=message, teachers=teachers)

@app.route('/attendance/update_status', methods=["POST"])
...
if __name__ == "__main__":
    app.run(debug=True)
            ...
            <td>
                <input name="attendance_status" type="radio" value="YES">Yes
                <input name="attendance_status" type="radio" value="NO">No
            </td>